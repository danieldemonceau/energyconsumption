...
describe('GET /usages - limit is not int', () => {
  test(`It should respond with an http 400, and 'Parameter limit is not zero or a positive integer'`, async () => {
    const response = await request(app).get(`/usages?apikey=${API_KEY_CLIENT_AUTH_ESCAPED}&limit=-1`);
    expect(response.status).toBe(400);
...
describe('GET /usages - offset is not int', () => {
  test(`It should respond with an http 400, and 'Parameter offset is not zero or a positive integer'`, async () => {
...
describe('GET /usages - from well formatted', () => {
  test(`It should respond with an http 200`, async () => {
...
describe('GET /usages - from not well formatted', () => {
  test(`It should respond with an http 400, and 'Parameter from is not in YYYY/MM/DD HH24:MI format'`, async () => {
...
describe('GET /usages - to not well formatted', () => {
  test(`It should respond with an http 400, and 'Parameter to is not in YYYY/MM/DD HH24:MI format'`, async () => {
